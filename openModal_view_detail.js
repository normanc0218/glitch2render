...
const openModal_view_detail = async (trigger_id, jobId) => {
  ...
  const blocks = [
    ...
    createTextSection(
      `*Assigned Staff:* ${
        Array.isArray(job.assignedTo) ? job.assignedTo.join(", ") : "N/A"
  ...
  if (job.endDate || job.endTime) {
    blocks.push(
      createTextSection(
        `*Done By:* ${job.updatedBy || "N/A"}\n*Cause of issue:* ${
          job.issueCauses || "N/A"
        }\n*Other reasons?:* ${job.otherreason || "N/A"}\n*Tools collected:* ${
          job.toolsCollected || "None"
        }\n*Lockout confirmed:* ${
          job.resetConfirmed
        }\n*Notify to Supervisor:* ${
          job.supervisorUser
        }\n*Message to Supervisor:* ${
          job.supervisorMessage || "None"
        }\n*Other Status:* ${
          job.otherStatuses || "None"
